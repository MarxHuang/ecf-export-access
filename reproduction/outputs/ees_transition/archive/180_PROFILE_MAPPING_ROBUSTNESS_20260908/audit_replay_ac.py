...
def audit_job(job):
    mapping,date=job;out=A/'days'/mapping/date
    ...
    if file.exists():
        ...
        return dict(mapping=mapping,date=date,reused=True,contradictions=len(old['contradictions']))
    ...
    return dict(mapping=mapping,date=date,seconds=report['seconds'],contradictions=len(contradictions))

def main(workers,limit):
    p=run.verify_sources()
    jobs=[(r['id'],d) for d in p['dates'] for r in p['mappings'] if (A/'days'/r['id']/d/'DONE.json').exists()]
    if limit:jobs=jobs[:limit]
    else:assert len(jobs)==400,'Full audit requires complete computation'
