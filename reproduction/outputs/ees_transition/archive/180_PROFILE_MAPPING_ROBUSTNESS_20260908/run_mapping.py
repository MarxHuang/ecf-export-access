"""Frozen primary code with observational capture for five placement permutations."""
...
def load_runner():
    spec=importlib.util.spec_from_file_location('mapping_primary_runner',F/'scientific_source/tools/run_ees_australian_external_replay.py')
...
def verify_sources():
    ...
    for r in p['mappings']: assert sha(Path(r['path']))==r['sha256'],r['id']
    return p
...
def run_day(job):
    ...
    prior=completed(out)
    if prior:return dict(mapping=name,date=date,resumed=True)
    out.mkdir(parents=True,exist_ok=True)
    if (out/'ERROR.json').exists():raise RuntimeError(f'Recorded error requires review: {out}')
    try:
        start=time.monotonic();m=load_runner();cfg=dict(p['config'])
        if name!='ORIGINAL_CHECK':cfg['mapping_file']=next(r['path'] for r in p['mappings'] if r['id']==name)
        m._worker_init(cfg)
        ...
        done=dict(protocol_sha256=sha(A/'PROTOCOL.json'),files={n:sha(out/n) for n in ['VECTORS.npz','RESULT.json']},
        ...
        return dict(mapping=name,date=date,seconds=done['elapsed_seconds'],physical_failed_calls=done['physical_failed_calls'])
    except Exception:
        error=dict(mapping=name,date=date,error=traceback.format_exc())
...
def run(workers):
    ...
    jobs=[(m['id'],d) for d in p['dates'] for m in p['mappings']]
    pending=[];complete_count=0
    for j in jobs:
        if completed(A/'days'/j[0]/j[1]):complete_count+=1
        else:pending.append(j)
    status=dict(pid=os.getpid(),started_utc=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),protocol_sha256=sha(A/'PROTOCOL.json'),total=400,already_complete=complete_count,workers=workers)
    ...
    verify_sources()
    assert complete_count==400
    (A/'COMPUTATION_COMPLETE.json').write_text(json.dumps(dict(days=400,protocol_sha256=sha(A/'PROTOCOL.json'),full_post_run_verification_pending=True)),encoding='utf-8')
