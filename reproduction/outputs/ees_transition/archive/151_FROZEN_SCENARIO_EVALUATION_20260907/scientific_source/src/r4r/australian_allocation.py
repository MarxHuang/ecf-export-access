...
class ProportionalAllocationResult:
    ...
    def pair_pass(self) -> bool:
        return (
            self.status in {"FULL_REQUEST_FEASIBLE", "FEASIBLE_ZERO_ANCHORED_BISECTION"}
...
def allocate_proportional_ac_prefix(
    ...
    *,
    ...
    """Find a tested feasible scale using zero-anchored bisection.
    ...
    If the full request fails, return the feasible lower bracket encountered
    by bisection. This routine does not prove connectedness or monotonicity of
    the AC feasible set and does not certify its global maximum. Independent
    full-ray checks are required to assess larger disconnected feasible sets.
    """
    ...
    return ProportionalAllocationResult(
        ...
        request_scale=lower,
        status="FEASIBLE_ZERO_ANCHORED_BISECTION",
        power_flow_count=power_flow_count,
        load_only_screen=load_only,
