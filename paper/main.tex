...
\begin{@twocolumnfalse}
 ...
 \raisebox{0pt}[0pt][0pt]{\includegraphics[height=55pt]{head_foot/RSC_LOGO_CMYK}}\\[1ex]
...
\includegraphics{head_foot/dates} & \noindent\normalsize
 ...
 how participants choose how much access to request before output is known. The analysis identifies
 ...
 ECF compares the completed allocation with an alternative that caps requests
 from participants with unused awards at their recorded delivery. Under the
 same network conditions and allocation rule, post-event estimates of available
 export show whether this alternative could have delivered more electricity
elsewhere and in total. Only a recovery that passes alternating-current (AC)
power-flow checks leads ECF to tighten that group's future ceilings;
 otherwise, ceilings recover toward their fixed connection bounds.
 Controlled medium-voltage simulations test five
 ...
 electricity. Fully replacing ECF's current ceilings with targets from the
 latest interval instead lowers delivery below the no-feedback level.
 The results show why future export access should reflect both the delivery
 that others could have supplied and the risk of restricting output recovery,
 rather than simply minimizing unused permission. \\
\end{tabular}
...
As distributed generation grows, operators need to make better use of existing
network capacity while planning reinforcement. Flexible export arrangements
...
controllability \cite{nea2025distributedpv}. These developments bring attention
to how access is managed after generators connect. Permission to export
...
Together with estimates of available output, completed operating records can
show whether a different allocation would have allowed more delivery. This
study uses that comparison to guide later request ceilings. Existing network limits,
...
Closed-loop operation uses measurements to revise subsequent actions in
cyber--physical power systems \cite{yu2016smartgridcps,xue2017cpsse}.
For export sharing, a completed record can similarly inform the next request
ceiling. Metered delivery reveals which awards were unused; available-export
information is needed to establish whether a different allocation could have
supplied more electricity. The update must also retain access for intervals
in which output recovers.
...
These approaches provide network-feasible allocation, incentives, and ways to
use operating history. Here, we ask whether unused export permission prevented
other participants from delivering electricity, and whether that finding
should change later request ceilings. This requires a comparison of possible
deliveries, beyond identifying a shortfall from awards and meter readings.
The comparison informs later requests while retaining the underlying allocation rule.
...
delivery elsewhere and increases the total. The update blends the current
ceiling with a target derived from the completed interval, while leaving the
allocation rule unchanged. We derive the ceiling bounds and recovery rate.
The comparison identifies recoverable delivery for the unused-award group as
a whole; it neither assigns individual responsibility nor requires private
payoff information.
...
fraction toward the average fraction in the ME allocation computed for the
same admitted requests and network conditions. It is
...
FA uses this ME reference solution only to compute
...
\end{equation}
With exact available-export records and the common tolerance used here, the
total-recovery condition implies the two outside-gain conditions (SI,
Section~S3.1). The latter describe how delivery is recovered; they are not
independent tests of the same event.
...
hold, ceilings for the unused-award group move toward delivery in that interval;
other ceilings move toward their fixed connection bounds. If any condition
fails, all ceilings move toward those bounds. The gain $\beta_{\mathrm{ECF}}$
sets the weight of this target in the next ceiling; the fixed connection
bound $C_i$ does not change.
...
Two controls separate where requests are reduced from whether to reduce them
(Table~\ref{tab:feedback_controls}). The equal-total control removes exactly as
much total request as ECF in each interval, but distributes that reduction
proportionally across participants. Its matched total comes from ECF, so it
explains the effect of request distribution rather than supplying an independent
operating policy. ECF without replay qualification instead keeps the same
ceiling-update formula and gain but applies the delivery target after every
unused award. This comparison tests the value of checking whether others
could have delivered more before tightening. The table also describes the
shortfall-cost, mismatch-only, and previous-day persistence comparisons.
...
of 9.784~kWp. In this scenario, each export connection bound $C_i$ is set
numerically equal to the participant's scaled PV capacity.
...
At each half-hour, the raw request equals the previous day's
$a_i^{\mathrm{prof}}$ at the same half-hour plus a nonnegative forecast increment.
The increment is the positive part of the 90th percentile of same-half-hour
day-to-day changes estimated from 2010--2011. The raw request is then capped
at the fixed connection bound $C_i$ before the feedback ceiling is applied.
This constructed forecast replaces the single-interval
...
recovery tests at error radii of 1\%, 5\%, and 10\% of the connection bound $C_i$,
...
participant locations, scales gross PV and the connection bounds by
...
Using that common baseline and the same raw requests, mismatch-only feedback
increases mean total delivery by 15.2~kW, while ECF increases it by 24.9~kW.
ECF therefore delivers 9.7~kW more than mismatch-only feedback.

Relative to forecast-based reference requests, the no-feedback loss of
21.7~kW becomes a gain of 3.2~kW under ECF. This improvement retains
79.4\% of the changed-request group's no-feedback delivery gain while reducing
the loss elsewhere. Section S3 reports the full group decomposition and
classification shares alongside both comparison baselines.
...
whereas the equal-total control fixes the amount removed.
...
The \suppref{} also reports secondary allocation-formulation analyses under
...
updates change relative requests instead. The energy balance records the
combined outcome; it does not assign separate causal effects to increased
authorization and reduced unused awards.
...
how often the model finds a recoverable opportunity; the daily comparisons
measure the subsequent delivery outcome.
...
checks, while network limits reduce the admitted-request scale in 44.4\% of
ECF half-hours. Thus the common operating limits still constrain allocation.
...
Keeping the update formula fixed isolates the value of the replay test.
Removing that test while retaining the same gain reduces modeled
...
ECF's mean advantage persists when participants revise requests from their own
delivery history. Across 80 dates, it delivers 1.44~MWh more than no feedback
and 4.53~MWh more than the same update without qualification. These totals
average three random seeds within each date. Participants choose among smaller
and larger requests, with preferences and ceilings carried across consecutive
days; these three controls develop separate request histories. The equal-total shadow
also delivers less than ECF, but follows ECF's history rather than learning
independently. Compared with fixed request-choice probabilities, preference
updates increase ECF's advantage over the update without qualification, but
do not establish a larger advantage over no feedback. Section S10.15 gives
the request model, all paired contrasts, and their uncertainty.
...
increase later delivery. One control tests recovery using the available-export
estimate directly. The other subtracts its error allowance, retaining at least
the completed delivery, to obtain a conservative estimate. We compare these
point-record and lower-bound controls at error radii of 1\%, 5\%, and 10\%
of the connection bound $C_i$, under common overestimation and underestimation.
Point-record control tightens after 11,
...
As in the placement comparison, increased delivery can accompany increased
unused authorization: both rise at half the primary PV capacity. Section S10.11
of the \suppref{} gives the complete energy balances, daily comparisons, and
physical checks. Placement and capacity therefore affect how much additional
electricity can use the changed allocation, not just how much permission remains unused.
...
Export-access arrangements should be evaluated by the electricity delivered,
not just the permission allocated or the fraction used. Both persistence and
the update without replay qualification improve measures of authorization use
while delivering less than ECF. The comparisons identify three practical
requirements: consider where requests are reduced, test whether an unused
award prevented delivery elsewhere, and preserve access for output recovery.
...
their fixed connection bounds. The bounded-record results also show the cost of caution:
...
\paragraph{Scope and field evaluation.}
...
An unused export award does not, by itself, establish a delivery loss elsewhere.
ECF tests this possibility by comparing feasible allocations for the completed
interval before adjusting later request ceilings. The comparison identifies
an opportunity for others to deliver more; the update must still preserve
useful access as output changes.
